class Product {
    ...
    constructor(name: string = 'Default name', price: number = 1, inStock: boolean = true){
        this.name = name,
        this.price = price,
...
class ChallengeProduct {
    ...
    public toString(): string {
        return `${this.name} costs ${this.price} (${this.descount}% off), final price ${this.getFinalPrice()}`
    }
...
Array(500).fill(0).forEach(() => {car.Acelerate()})
console.log(car.Acelerate());
Array(500).fill(0).forEach(() => {car.Brake()})
